...
static int sata_drive_check_power_management(int node, int sata)
{
	...
	sctl.u = CSR_READ(node, CAVM_SATAX_UAHC_P0_SCTL(sata));
	...
	if (sctl.s.ipm != (PM_DISABLE_SLUMBER | PM_DISABLE_PARTIAL)) {
		...
		debug("ATF: N%d: SATA%d: Force disabling PARTIAL and SLUMBER\n",
		      node, sata);
		CSR_WRITE(node, CAVM_SATAX_UAHC_P0_SCTL(sata),
			  sctl.u);
	}
	return 0;
}
...
static int sata_drive_check_detect_failure(int node, int sata)
{
	 ...
	 */
	cmd.u = CSR_READ(node, CAVM_SATAX_UAHC_P0_CMD(sata));
	...
	ssts.u = CSR_READ(node, CAVM_SATAX_UAHC_P0_SSTS(sata));
	is_detected = (ssts.s.det == SSTS_DET_PHY_DEV_GOOD);

	sig.u = CSR_READ(node, CAVM_SATAX_UAHC_P0_SIG(sata));
	...
	tfd.u = CSR_READ(node, CAVM_SATAX_UAHC_P0_TFD(sata));
...
static int sata_drive_check_unplug_failure(int node, int sata)
{
	...
	for (i = 0; i < NUM_CONTINUOUS_EIE_READ; i++) {
		detsts.u = CSR_READ(node, CAVM_GSERX_RX_EIE_DETSTS(qlm));
	...
	serr.u = CSR_READ(node, CAVM_SATAX_UAHC_P0_SERR(sata));
	if (serr.u != 0) {
		/*
		 * The controller knows there isn't a drive, so we
		 * don't need to do anything.
	...
	CSR_WRITE(node, CAVM_GSERX_LANEX_LBERT_CFG(qlm, lane), lbert_cfg.u);

	/* Put the PCS in internal loopback */
	cfg_0.u = CSR_READ(node, CAVM_GSERX_LANEX_MISC_CFG_0(qlm, lane));
	cfg_0.s.cfg_pcs_loopback = 1;
	CSR_WRITE(node, CAVM_GSERX_LANEX_MISC_CFG_0(qlm, lane), cfg_0.u);
	CSR_READ(node, CAVM_GSERX_LANEX_MISC_CFG_0(qlm, lane));
	...
	CSR_WRITE(node, CAVM_GSERX_LANEX_LBERT_CFG(qlm, lane), lbert_cfg.u);
	CSR_READ(node, CAVM_GSERX_LANEX_LBERT_CFG(qlm, lane));

	/* Clear non-recovered persistent communication error */
	CSR_WRITE(node, CAVM_SATAX_UAHC_P0_SERR(sata), 0x200);
	...
	for (node = 0; node < node_cnt; node++) {
		...
		for (sata = 0; sata < sata_ctrlr_count; sata++) {
			...
			do {
				...
				cmd.s.pod = 1;
				cmd.s.sud = 1;
				CSR_WRITE(node, CAVM_SATAX_UAHC_P0_CMD(sata), cmd.u);
				...
				do {
					serr.u = CSR_READ(node, CAVM_SATAX_UAHC_P0_SERR(sata));
					CSR_WRITE(node, CAVM_SATAX_UAHC_P0_SERR(sata), serr.u);
				...
				is.u = CSR_READ(node, CAVM_SATAX_UAHC_P0_IS(sata));
				CSR_WRITE(node, CAVM_SATAX_UAHC_P0_IS(sata), is.u);

				clb.u = (uintptr_t)ahci_cbl;
				CSR_WRITE(node, CAVM_SATAX_UAHC_P0_CLB(sata), clb.u);

				fb.u = (uintptr_t)ahci_rfis;
				CSR_WRITE(node, CAVM_SATAX_UAHC_P0_FB(sata), fb.u);
				...
				ci.s.ci = 1 << 0;
				CSR_WRITE(node, CAVM_SATAX_UAHC_P0_CI(sata), ci.u);

				timeout = 1000;

				do {
					udelay(1);
					ci.u = CSR_READ(node, CAVM_SATAX_UAHC_P0_CI(sata));
