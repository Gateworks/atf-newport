			...
			unsigned int *flags)
{
	...
	assert(key_len != NULL);
	assert(flags != NULL);
	assert(plat_octeontx_bcfg->bcfg.trust_rot_addr != 0);
	...
	memcpy(ptr, (unsigned char *)plat_octeontx_bcfg->bcfg.trust_rot_addr, ROTPK_BYTES);
	...
	words = 4;
	src = (uint64_t *)plat_octeontx_bcfg->bcfg.trust_rot_addr;
	for (i = 0; i < words; i++) {
		tmp = src[words - 1 - i];
	...
	src = (uint64_t *)(plat_octeontx_bcfg->bcfg.trust_rot_addr + ROTPK_BYTES/2);
	for (i = 0; i < words; i++) {
		tmp = src[words - 1 - i];
		*ptr++ = (unsigned char)((tmp >> 56) & 0xFF);
		*ptr++ = (unsigned char)((tmp >> 48) & 0xFF);
...
int plat_get_crypt_key(unsigned char **key, unsigned int *key_len)
{
	...
	ptr = &aes_key[0];
	if (plat_octeontx_bcfg->bcfg.trust_key_addr != 0) {
		/* Try to get the BSSK key */
		memcpy(ptr, (unsigned char *)plat_octeontx_bcfg->bcfg.trust_key_addr, AES_KEY_BYTES);
	} else {
		/*
